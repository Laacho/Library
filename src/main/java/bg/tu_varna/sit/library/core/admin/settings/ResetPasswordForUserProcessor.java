...
public class ResetPasswordForUserProcessor extends BaseProcessor implements ResetPasswordForUserOperationModel {
    ...
    public Either<Exception, ResetPasswordForUserOutputModel> process(ResetPasswordForUserInputModel input) {
        return Try.of(()->{
                    ...
                    ResetPasswordForUserOutputModel build = outputBuilder();
    ...
    private ResetPasswordForUserOutputModel outputBuilder() {
        return ResetPasswordForUserOutputModel.builder().message("Successfully reset password for user!").build();
    }
}
